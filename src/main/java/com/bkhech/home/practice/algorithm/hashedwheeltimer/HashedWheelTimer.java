...
public class HashedWheelTimer implements Timer {
    ...
    /**
     * HashedWheelTimer对ticksPerWheel正则化, 将其转化为大于等于该值的2^n的最小值
    ...
    private final class Worker implements Runnable {
        ...
        public void run() {
            ...
            do {
                //该方法内部sleep线程，直到指针指向下一个扇形块，方法返回
                ...
                if (deadline > 0) {
                    ...
                    HashedWheelBucket bucket =
                            wheel[idx];
                    //调用HashedWheelBucket#addTimeout将队列中的任务放到时间轮上
                    transferTimeoutsToBuckets();
                    //执行扇形块对应的任务或者将任务的圈数减一
                    bucket.expireTimeouts(deadline);
                    tick++;
                }
            } while (WORKER_STATE_UPDATER.get(HashedWheelTimer.this) == WORKER_STATE_STARTED);//判断时间轮是否终止
            ...
            //将队列中未处理的任务放到unprocessedTimeouts中，外部调用者可以通过unprocessedTimeouts得到未执行的任务
            for (; ; ) {
